...
import recipeComponents from "../../lib/portableText/recipe/recipeComponents";
...
export default function Post(props: BlogPost) {
  ...
  return (
    <>
      ...
      <main>
        <h1>{blog.tittel}</h1>
        <PortableText value={blog.content} components={recipeComponents} />
      </main>
      {blog.cudoz && (
...
export async function getStaticPaths() {
  const blogs = await client.fetch(
    `*[_type=="oppskrift" && defined(slug)]{"slug":slug.current}`
  );
...
export const getStaticProps: GetStaticProps<{}> = async (ctx) => {
  ...
  const blog = await client.fetch(
    `*[_type=="oppskrift" && slug.current=="${slug}"]{
	"ikiId": _id,
